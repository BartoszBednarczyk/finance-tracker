...
@Component({
    selector: 'app-expenses-by-categories-pie',
    templateUrl: './expenses-by-categories-pie.component.html',
    styleUrls: ['./expenses-by-categories-pie.component.scss'],
})
export class ExpensesByCategoriesPieComponent implements OnInit {
    chartOptions: any;
    transactions: any[] = [];
    ...
    constructor(private _transactionsService: TransactionsService, private _categoriesService: CategoriesService) {
        ...
        this._transactionsService.transactions.pipe(debounceTime(500)).subscribe((transactions) => {
            if (transactions) {
                this.transactions = _transactionsService.sortTransactions(transactions, { order: 'Asc', query: 'date' });
                this.addDataToChart();
                console.log('Halo');
            }
        });
